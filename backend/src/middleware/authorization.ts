...
class UserController {
    ...
    async createNewUser(req: Request, res: Response) {
        ...
        if (!fullname || !username || !password)
            return noGoodRequest(res, "missing fields");

        const existing = await UserRepository.findOneBy({ username });
        if (existing)
            return noGoodRequest(res, "username already used");
    ...
    async login(req: Request, res: Response) {
        ...
        if (
            !loginUser ||
            !(await loginUser.comparePassword(base64ToString(password)))
        ) {
            return noGoodRequest(res, "username/password incorrect");
        }
